...
5. 导入完成后，日历只启用数据日期范围内的日期；第一次点击选择开始日期，第二次点击选择结束日期，按闭区间导出。点击清除日期范围则导出全部日期。
6. 选择分区配置和输出目录。
7. 点击“开始拆分导出”。

程序按批次写入 SQLite，导出时使用磁盘临时分组表和 `openpyxl` 流式工作簿，避免把全量数据放进 Python 内存，适合千万级总行数（实际耗时和磁盘空间取决于列数、客户数和输出文件数量）。单个客户超过 Excel 单表行数限制时会自动生成 `_part2.xlsx` 等分卷文件。
