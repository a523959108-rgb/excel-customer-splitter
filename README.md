...
程序会用多个读取线程并行解析多个 Excel 文件，再通过单一 SQLite 写入通道按批次落库。SQLite 保持单写入通道可以避免多个线程争抢写锁；默认最多使用 4 个读取线程。导出时使用磁盘临时分组表和 `openpyxl` 流式工作簿，避免把全量数据放进 Python 内存，适合千万级总行数（实际耗时和磁盘空间取决于列数、客户数和输出文件数量）。单个客户超过 Excel 单表行数限制时会自动生成 `_part2.xlsx` 等分卷文件。导入或导出过程中可以点击“结束任务并释放资源”，程序会协作停止当前任务、清理临时表和工作簿，并重新建立 SQLite 连接。
